...
if(isset($_SESSION["username"])){
    if(isset($_POST["data_id"])){
        ...
        if ($row["count"] == 0) {
            ...
            $res = mysqli_query($conn,"INSERT INTO block_channels (`id_channel`,`username`,`url_chanel`,`channel_name`) VALUES ('$chanel_id','$username','$chanel_url','$title')");
            if($res){
                $data["status"] = true;
                $data["message"] = "Chặn kênh thành công!";
            }
            else{
                $data["status"] = false;
                $data["message"] = "Có lỗi khi chặn kênh!";
            }
        }
    }
}
